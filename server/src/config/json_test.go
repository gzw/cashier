...
var testjsonconf string = "testjson.conf"

func TestJson(t *testing.T) {
	f, err := os.Create(testjsonconf)
	if err != nil {
		t.Fatal(err)
	}
	...
	f.Close()
	defer os.Remove(testjsonconf)
	jsonconfig, err := NewConfig("json", testjsonconf)
	if err != nil {
		t.Fatal(err)
	}

	fmt.Println(jsonconfig.String("appname"))
	fmt.Println(jsonconfig.Int("httpport"))
	fmt.Println(jsonconfig.Float("PI"))
	fmt.Println(jsonconfig.Bool("autorender"))
	fmt.Println(jsonconfig.Int("database::conns::maxconnection"))
}
